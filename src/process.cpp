...
namespace Base
{
	...
	std::wstring getModuleFullPath(HANDLE hProcess, HMODULE hModule)
	{
		...
		while (true)
		{
			DWORD pathSize = GetModuleFileNameEx(hProcess, hModule, (wchar_t*)buffer.data(), DWORD(buffer.size() + 1));
			if (pathSize == 0)
				return std::wstring();
			if (pathSize == buffer.size())
			{
				size_t new_size = buffer.size() * 2;
				if (new_size - 1 > std::numeric_limits<DWORD>::max()) new_size = std::numeric_limits<DWORD>::max() - 1;
