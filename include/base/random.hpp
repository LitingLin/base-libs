...
namespace Base
{
	...
	public:
		// [a, b]
		template <typename Type, typename = std::enable_if_t<std::is_same_v<Type, short> || std::is_same_v<Type, int> || std::is_same_v<Type, long> ||
		...
		template <typename Type, typename = std::enable_if_t<std::numeric_limits<Type>::is_integer &&
			...
			Type get(Type min = std::numeric_limits<Type>::min(), Type max = std::numeric_limits<Type>::max())
		{
			std::uniform_real_distribution<short> distribution(min, max);
